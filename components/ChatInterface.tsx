    ...
    const handleSend = async () => {
        ...
        try {
            ...
            const errorMsg: ChatMessage = {
                ...
                role: 'model',
                content:
                    'I encountered an error connecting to the AI service. Please ensure your API key is configured correctly.',
                timestamp: new Date(),
                isError: true,
            };
        ...
        <div className="flex flex-col h-screen max-h-screen bg-slate-950 relative overflow-hidden">
            ...
            <header className="px-8 py-5 border-b border-white/5 bg-slate-950/70 backdrop-blur-xl z-20 sticky top-0 flex items-center justify-between shadow-lg shadow-black/20">
                <div>
                    ...
                    <p className="text-xs text-slate-500 mt-1 pl-[54px]">TaxIntegrity Audit Protocol v2.5</p>
                </div>
            ...
            <div className="flex-1 overflow-y-auto p-4 md:p-8 space-y-8 z-10 relative scrollbar-hide">
                {messages.map((msg) => (
                    <div
                        key={msg.id}
                        className={`flex items-start gap-5 max-w-4xl mx-auto group ${msg.role === 'user' ? 'flex-row-reverse' : 'flex-row'
                            }`}
                    >
                        {/* Avatar */}
                        <div
                            className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 shadow-lg ${msg.role === 'user'
                        ...
                        <div className={`flex flex-col ${msg.role === 'user' ? 'items-end' : 'items-start'}`}>
                            <div
                                className={`px-6 py-5 rounded-2xl max-w-2xl leading-relaxed shadow-xl transition-all ${msg.role === 'user'
